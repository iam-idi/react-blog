...
const UpdateBlog = () => {
  ...
  let handleSubmit = async(e) => {
    e.preventDefault();
    try {
      let res = await fetch('http://localhost:8000/blogs/' + id, {
        method: 'PUT',
        headers: {
      ...
      toast.success('Blog post updated successsfully');
  ...
  return (
    <div className="py-6 sm:py-10 px-2 sm:px-6">
      <h2 className="text-lg sm:text-2xl font-bold text-stone-400 mb-5 sm:mb-8 text-center">Update Blog</h2>
      <form action="" onSubmit={handleSubmit} className="my-0 mx-auto w-fit">
        ...
        <button className="block text-sm font-bold sm:text-base text-stone-500 mt-4 mx-auto sm:mt-8 focus:border-blue-400 rounded px-1 sm:px-2 min-w-[150px] sm:min-w-[200px] w-[30vw] bg-blue-400 h-[35px] sm:h-[45px] transform active:scale-95">Update Blog</button>
      </form>
    </div>
  )
}
